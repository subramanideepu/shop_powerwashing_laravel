...
{
    // 🔥 Flash sale = no margin
    if (FlashSale::contains($this->product)) {
        return $this->specialPrice() ?? $this->product->price;
    }

    // ✅ Apply margin-aware price
    $finalAmount = calculate_margin_price($this->product);

    return Money::inDefaultCurrency($finalAmount);
}
